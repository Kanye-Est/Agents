...
\begin{column}{0.5\textwidth}
  \begin{itemize}
    ...
    \item 它在一个循环里工作：\textbf{感知 $\to$ 思考 $\to$ 行动(调用工具) $\to$ 观察 $\to$ 再思考}。
...
\begin{column}{0.54\textwidth}
  \textbf{Skill = 给智能体扩展能力的“外挂”}，多来自第三方/社区。两种主流形态：
  \begin{itemize}
    \item \textbf{SKILL.md 式}：一个\textbf{文件夹} = \texttt{SKILL.md}（说明书：名字+描述+用法）+ \texttt{handler.py}（被调用时执行的脚本）。
    \item \textbf{MCP server 式}：一个\textbf{独立进程}，智能体连上去发现并调用它的工具。
  \end{itemize}
  \begin{alertblock}{关键}
    这些“外挂”是\textbf{别人写的}。如果其中一个是坏人写的呢？
  \end{alertblock}
\end{column}
\end{columns}
\end{frame}
...
agent = build_agent(extra_tools=[skill])       \# 描述进系统提示, 工具即可被调用
  \end{semiverbatim}
  \end{block}
  \footnotesize 注意两个“安装副作用”：(1) skill 的\textbf{描述}会被拼进\textbf{系统提示}（影响 LLM 决策）；
  (2) skill 的\textbf{脚本}会以智能体\textbf{进程权限}运行。——这正是后面攻击的两个入口。
\end{frame}
...
\begin{frame}{实验装置（全程受控 / 合成假数据）}
  ...
  \begin{exampleblock}{伦理边界}
    本地沙箱、合成假数据、外传只发往\textbf{本机} mock 接收端；仅用于\textbf{防御性研究}。
  \end{exampleblock}
  \end{column}
  \end{columns}
  \vfill\centering\footnotesize 下面每类攻击都给这个助理\textbf{挂一个第三方 skill}——先看 skill 怎么写、怎么装，再看效果。
\end{frame}
...
\end{semiverbatim}
\end{alertblock}
\footnotesize 安装：\texttt{agent = build\_agent(extra\_tools=[load\_skill(".../t1\_prompt\_injection")])}\\
...
\end{semiverbatim}
\end{alertblock}
\footnotesize 利用了 \texttt{TerminalTool} 自称“安全只读”却 \texttt{shell=True}、白名单含 \texttt{bash} 的缺陷。\\
...
\begin{frame}{根因：四个“默认信任”}
  ...
  \end{enumerate}
  \vfill\centering\alert{共性：智能体把“skill”当可信，而它本就来自不可信的第三方。}
\end{frame}
...
\begin{column}{0.5\textwidth}\textbf{最小权限与可观测：}
  \begin{itemize}
    ...
    \item 危险操作\textbf{人工确认(HITL)}+真正命令过滤（防 T4）
...
\begin{frame}{总结}
  \begin{itemize}
    ...
    \item 四类攻击实证（注入/外泄/投毒/越权）\textbf{4/4 成功}，且 skill 外表\textbf{人畜无害}、门槛极低。
    \item 根因是一连串\textbf{“默认信任”}——这正是从“能用”走向“可信”要补的课。
  \end{itemize}
  \vfill\begin{block}{一句话 takeaway}
