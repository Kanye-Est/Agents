...
1. **未获授权即视为停止**：本协议 PENDING（§0）。取得针对本文件的书面授权前，`run 数必须为 0`。
...
（生效记录仍 **暂无**。本协议 **PENDING 授权**；获权前不得有任何 run。授权到手后，导师确认原文单独归档并哈希、
追加**生效记录**、生成生效后协议 `sha256`，方可按 §6.3 逐次执行。此后任何修改按 idea/42 §10 走
append-only amendment，记录日期、理由与当时已见证据。）

---

### PENDING EFFECTIVE RECORD —— 范围声明（草案，**未生效**）

> 状态：**草案 · 未生效 · 运行次数 = 0**。日期占位：2026-09-09 会议纪要拟经归档生效。
> **本块不解除授权门。** 生效以下述两枚**仓库外归档件哈希**填入为准；填入前，本协议按 §0/§14 仍 PENDING、run 数 = 0。
> 锚定**外部归档件**（非 idea/52 自身哈希），以避免"记录引用其所在文件"的循环。

**【授权门 —— 两枚哈希，均为用户归档动作，此处占位】**

- **(H1) 会议纪要归档件** `sha256 = <PENDING：纪要另存为独立文件后填入>`（依决议"以本纪要归档件含 SHA256 为准"）。
- **(H2) 冻结的 Amendment 1 协议快照** `sha256 = <PENDING：将含 Amendment 1 的协议快照另存归档后填入>`（依 A1.8"生效记录须显式引用 Amendment 1 的 sha256"）。
- **两枚哈希均填入且导师确认后**，本块状态改 `EFFECTIVE`、填生效日期，方按 §6.3 逐次开跑。**在此之前 run 数硬性 = 0。**
...
> 日期：2026-09-09 · 状态：**DRAFT，PENDING EFFECTIVE RECORD · 运行次数 = 0**
...
- 本 Amendment 仍受 §9 全部停止条件约束；§0 "PENDING 授权 → run 数必须为 0" 在生效记录落定前继续成立。
