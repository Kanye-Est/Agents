...
def s_title():
    ...
    txt(s, 1.0, 6.35, 11, 0.4, [[("汇报人：Kan Shiyu    2026-07-29", 13, LGRAY, False)]])
...
def s_divider(kicker, title, sub, color, genre=None):
    s = prs.slides.add_slide(BLANK)
    rect(s, -0.1,-0.1, SW+0.2, SH+0.2, color)
    txt(s, 1.0, 2.45, 11.3, 0.6, [[(kicker, 20, RGBColor(0xFF,0xFF,0xFF), True)]])
    txt(s, 1.0, 3.0, 11.3, 1.3, [[(title, 40, WHITE, True)]])
    rect(s, 1.04, 4.28, 2.2, 0.05, RGBColor(0xFF,0xFF,0xFF))
    txt(s, 1.02, 4.42, 11.3, 0.7, [[(sub, 19, RGBColor(0xE8,0xEE,0xF6), False)]])
    if genre:
        rect(s, 1.04, 5.55, 0.13, 0.52, WHITE)
        txt(s, 1.33, 5.55, 11.0, 0.52, [[("体裁 GENRE　", 15, RGBColor(0xE8,0xEE,0xF6), True),
                                          (genre, 17, WHITE, True)]], anchor=MSO_ANCHOR.MIDDLE)
    return s
...
s_divider("主讲 · MAIN", "“Do Not Mention This to the User”",
          "Detecting & Understanding Malicious Agent Skills in the Wild · USENIX Security 2026 · Yi Liu et al.", BLUE,
          genre="in-the-wild 大规模测量研究（不是 benchmark）"); n=5
...
s_img("三个发现从哪来：漏斗产出语料 → 三种追问", "fig_dnm_rq.png", accent=BLUE, kicker="核心结论 · RQ 的由来",
      tk="RQ 不是新实验：都是对漏斗筛出的那 157 个恶意语料，做三种不同的统计追问。", tkcolor=BLUE,
      note="这页专门回答“三个结论是怎么从骨架得出来的”。记住这篇是 in-the-wild 测量研究，不是 benchmark——"
           "它没有预设任务、没有 P0/P1 对照，而是先用漏斗（98,380→4,287→762→157）筛出一份高置信度的 ground-truth 语料：157 个行为确认的恶意 skill，"
           "标注了 632 个漏洞、13 种技术、6 个 kill-chain 阶段。三个 RQ 就是拿着这同一份语料，从三个角度做统计：\n"
           "RQ1（威胁全貌）问“这批语料整体长什么样”——平均每个含 4.03 个漏洞、84.2% 的漏洞就写在 SKILL.md 的自然语言里，说明是蓄意分层。\n"
           "RQ2（攻击者与分工）问“谁在做、怎么分工”——统计检验发现 Data Thieves 与 Agent Hijackers 两种原型负相关（攻击者二选一专精），且单一工业化行为者用模板化仿冒贡献 54.1%。\n"
           "RQ3（如何逃避）问“越高级的攻击是不是越隐蔽”——shadow feature 比例随攻击复杂度从 0% 升到 100%，还会武器化平台自带的信任机制。\n"
           "所以三个结论不是三份新数据，而是同一份漏斗产物上的三次不同切法——这正是测量研究的典型结构。", n=8); n=9
s_text("它回答了什么，没回答什么", [
    [("✓  回答了：", 24, GREEN, True)],
...
s_divider("补充① · SUPPLEMENT", "SCR：Benign in Isolation, Harmful in Composition",
          "Skill Composition Risk · arXiv 2606.15242 · 华东师大 / A*STAR（预印本）", TEAL,
          genre="受控实证 benchmark（SCR-Bench，mock 沙箱）"); n=11
...
s_divider("补充② · SUPPLEMENT", "HalluSquatting：Beware of Agentic Botnets",
          "抢注 AI 的“幻觉” · arXiv 2607.07433 · Tel Aviv U / Technion / Intuit（预印本）", RED,
          genre="测量 + 真实产品上的受控端到端攻击"); n=15
...
s_divider("我的 idea · MY WORK", "三篇 → 我要测的“同一条轨迹”",
          "Task-induced Autonomous Acquisition：普通任务能否自主接通 发现→安装→调用", VIO,
          genre="受控实证 benchmark（导师建议的方向）"); n=21
s_img("三篇各覆盖一段 —— 但同一条轨迹没人测", "fig_integration.png", accent=VIO, kicker="整合 · 提出问题",
      tk="三篇分别做了“供给 / 促装 / 执行”；缺的是“普通任务→自主全链→逐段归因”这条同一轨迹。", tkcolor=VIO,
      note="这是承上启下的一张图。把 agent 获取 skill 的完整生命周期横着摆开：能力缺口→外部发现→发出安装→解析执行→落盘→注册→调用→载荷。"
           "Do Not Mention 在最左——市场里有恶意供给在等着；SCR 覆盖“发出→执行”，但在 mock 环境、从已有安装请求起步；"
           "HalluSquatting 覆盖“显式 install→落盘→调用→RCE”，但用户已有获取意图。"
           "没有一篇把最下面那条紫色路径——普通任务触发、目标未装、agent 自主走完整条链、逐段测断点——放进同一条轨迹。"
           "留个悬念：图里那个问号我原本压在“发出→执行”这一跳，觉得最容易断在这；后面结果会告诉你我猜错了。", n=21); n=22
s_img("我的方案：把这条链做成一个实证 benchmark", "fig_my_benchmark.png", accent=VIO, kicker="方法 · 导师建议的方向",
      tk="测量仪（漏斗）＋数据集（twin）＋预注册协议（P0/P1/P2）—— 让风险可复用、可定位、可归因。", tkcolor=VIO,
      note="导师建议我照 empirical benchmark 的路子写，这页就是答卷：三件套。"
           "一是测量仪——Acquisition Funnel，把风险拆成 8 段逐点条件存活率，还有 3 个互斥终态，让“断在哪一段”成为可测对象，而不是报一个笼统 ASR。"
           "二是数据集——30 个 hard 任务 × PDF/iCal/QR 三类，成败由确定性 verifier 判定、不靠模型自评；每个任务有良性/恶意一对 twin，模型能看到的字段字节级完全相同。"
           "三是协议——预注册 P0 中性 / P1 鼓励获取 / P2 执行层 gate，单变量因果操纵，先把判据冻结、连 manifest 和源码 hash 都提交，再看结果。"
           "三件套凑齐，才谈得上是能复用、能定位断点、能做因果归因的 benchmark——这正是导师说的方向。", n=22); n=23
s_img("主结果：第一跳才是瓶颈（我猜错的地方）", "fig_my_result.png", accent=VIO, kicker="结果 1 · Primary",
      tk="P0 5/30 → P1 24/30（+63pp, p=3.8e-6）；瓶颈不在“发出→执行”，而在“任务→要不要搜索”。", tkcolor=VIO,
      note="这是主结果，也是那个悬念的揭晓。anchor cell（Qwen×文本）：中性策略下 30 个 hard 任务只有 5 个自主走完全链；"
           "只把 scaffold 的措辞换成“工具不够就去搜、去装、去调用”，就跳到 24 个，+63.3 个百分点，配对 McNemar p=3.8e-6，PDF/ICS/QR 三类同向。"
           "反直觉在曲线形状：我原以为会断在“发出→执行”，结果那一跳几乎不掉——真正、也是唯一的大衰减在第一跳 task→search，从 5 涨到 25。"
           "一旦 agent 决定去搜，检索、推荐、安装、落盘、调用、载荷近乎必然贯通到底。所以风险的开关，是“它把普通任务解释成要不要去找外部能力”。"
           "别忽略蓝线：中性策略 P0 仍有 5/30 自主 E2E——工具可得本身就非零风险。绿线（P2）留到下一页讲防御。强调：这是固定 benchmark 上的率，不是现实世界 prevalence。", n=23); n=24
s_img("防御：gate 挡得住“装不装”，挡不住“该不该装”", "fig_my_defense.png", accent=VIO, kicker="结果 2 · Defense",
      tk="P2 gate 拦下 25/25 次静默安装；但批准一个“无法区分的恶意 twin”，效用和载荷一起回来 —— gating ≠ vetting。", tkcolor=VIO,
      note="防御这页是一条弧线。把 approval gate 放在解析之后、执行之前：P1 自动获取时攻击和效用都高；P2 拒绝时每次安装都被拦（25/25），安全但零效用。"
           "然后批准一个良性 skill——效用回来 25/30、载荷为 0，理想。但把良性换成一个模型可见字段字节级完全相同的恶意 twin，再批准一次——效用还是 25/30，"
           "载荷也回到 25/30。也就是说：gate 决定“能不能装”，决定不了“该不该装”，因为批准者和 agent 看到的信息一样多。"
           "结论不是“gate 没用”，而是“gate 只是必要不充分”：完整防御还要叠认证 provenance、静态/信誉审查和最小权限。", n=24); n=25
s_img("稳健性：换模型/换 scaffold，攻击都在，变的是量级", "fig_my_general.png", accent=VIO, kicker="结果 3 · Generalization",
      tk="攻击本身、第一跳瓶颈、gate 防御都复现；但严格跨-family 判据只 anchor 通过 —— 诚实报边界。", tkcolor=VIO,
      note="第三块是稳健性，把话说满也把边界说清。复现的三件事：P1 下“获取+真实载荷”在 Llama 和 Qwen-原生FC 上都升（Llama 甚至 install+payload 8→28）；"
           "第一跳瓶颈复现——一旦搜索下游必然到底；P2 gate 每格都 100% 拦截。会变的两件事：功能主指标依赖模型，我预注册的严格跨-family 判据只有 anchor 格通过，"
           "Llama 的 QR、Qwen-原生的 ICS 各有一个 family 反向，所以我只能说“pooled 效应为正”，不能说“严格跨-scaffold 已成立”。"
           "而且要说清：native FC 只是换了调用表面、不是换了真实产品框架，所以这里说的是“跨调用表面”稳健，“跨真实产品”还没测。"
           "最反直觉的一条请大家记住：Llama 触发的 payload 更多（28）却完成更少任务（9）——只用“任务是否成功”当指标，会把更受害的模型评成更安全。"
           "所以 payload 与任务效用必须分开报告，这本身就是这个 benchmark 的一个方法学结论。", n=25); n=26
s_text("讨论 & 下一步", [
    [("现状：预注册的因果 benchmark 已经跑通，主结果 + 防御 + 初步 generalization 都在手。", 20, VIO, True)],
    [("", 6, INK, False)],
    [("最该补的边界 · 现实性尚未定锚", 20, RED, True)],
    [("   harness / 市场 / twin / P1 策略都是自建合成的——现在能证明“这种自我扩展策略会导致自主获取+执行”，", 17, INK, False)],
    [("   还不能证明“现实主流产品普遍这么配”。（native FC 只换了调用表面，不是第二个真实产品框架。）", 17, INK, False)],
    [("", 6, INK, False)],
    [("下一步", 20, VIO, True)],
    [("   1. 现实性定锚：把 P1 对标真实 agent 产品的自我扩展措辞、接一个真实框架，从“免责口径”升成“实测支撑”；", 17, INK, False)],
    [("   2. Variance：anchor × 多 seed × 温度配稳定性区间；补 frontier 闭源模型看“第一跳瓶颈”是否仍成立；", 17, INK, False)],
    [("   3. 把“payload 与任务效用解耦”写成一条独立的评测建议。", 17, INK, False)],
    [("", 6, INK, False)],
    [("想请大家讨论", 20, TEAL, True)],
    [("   Q1  作为 benchmark paper，哪条最该当主线：第一跳瓶颈 / gating≠vetting / 解耦指标？", 17, INK, False)],
    [("   Q2  严格跨-family 判据只 anchor 通过 —— 是弱化判据，还是把“异质性”本身当结果报？", 17, INK, False)],
], accent=VIO, kicker="收尾",
   note="收尾。现状是：预注册的因果 benchmark 已经跑通，主结果、防御、初步 generalization 都有了，不再是“打算做 pilot”。"
        "但要主动亮一条最该补的边界——现实性尚未定锚：harness、市场、twin、P1 策略全是我自建合成的，"
        "所以现在证明的是“这套自我扩展策略会因果地导致自主获取+执行”，还没证明“现实主流产品普遍这么配”；"
        "native FC 也只是换了调用表面，不是第二个真实产品框架。"
        "下一步三件：第一优先是把 P1 对标真实产品措辞、接真实框架，给现实性定锚；再给 anchor 配 variance、补前沿闭源模型；"
        "把“payload 与任务效用必须分开”写成独立评测建议。"
        "两个讨论题：一是三条结论里哪条最该当论文主线；二是严格跨-family 判据只 anchor 通过，是弱化判据还是把异质性本身当结果报。谢谢，请老师和大家指点。", n=26); n=27
# backup: 口径红线
s_text("（备用）汇报口径红线", [
    ...
    [("   • 我的率是固定 benchmark 上的率，不是现实世界 prevalence；严格跨-family 判据只 anchor 格通过。", 18, INK, False)],
    [("   • 别把 P1 说成“现实主流普遍配置”：harness/市场/twin/P1 全是自建合成，证明的是策略的因果后果、不是现实普遍性。", 18, INK, False)],
    [("   • payload 与任务效用要分开报：Llama payload 28 但 task_ok 只 9，别把它当“更安全”。", 18, INK, False)],
    [("   • 三篇百分比不能直接横比：起点、终点、成功定义都不同。", 18, INK, False)],
    [("   • research gap ≠ novelty：贡献来自新测量 / 新现象 / 方法 / 防御，率低也要有结构才有价值。", 18, INK, False)],
], accent=GRAY, kicker="BACKUP",
   note="这页备用，被追问时用。关键红线：不抢“第一个”；26.1% 别错当本篇；自己的率是固定 benchmark 不是 prevalence、严格判据只 anchor 过；"
        "别把 P1 说成现实主流普遍配置（全是自建合成，证明的是因果后果不是普遍性）；"
        "payload 与任务效用分开报；三篇不横比；gap 不等于贡献。", n=27)
